      ...
      <Col style="width:100%">
        <top-card class="orginalmanage-topcard">
          ...
          >
            <Row
              :gutter="15"
              style="margin-bottom:20px;border-bottom:1px solid #DFE5F2"
              type="flex"
            >
              ...
              <Col :span="16">
                ...
                <FormItem label="会议类型" prop="lifeType">
                  ...
                  <Select v-model="formValidate.lifeType" placeholder="请选择会议类型">
                    <Option
                      v-for="item in lifeTypeDown"
                      :value="item.value"
                      :label='item.label'
                      :key="item.value"
                    >{{ item.label }}</Option>
...
import { DatePicker, Select, RadioGroup, Radio, Option } from "iview";
  ...
  mounted() {
    ...
    if (id) {
      GetMyOrganLife({ input: id }).then(res => {
        ...
        this.formValidate.startTime = getNowFormatDate(
          this.formValidate.startTime
        );
        this.formValidate.endTime = getNowFormatDate(this.formValidate.endTime);
      });
    }
    console.log(this.lifeTypeDown)
    this.lifeTypeDown = this.$store.state.baseCode.LifeTypeEnum;
  },
  methods: {
    changeOrg(n) {
      this.org = n;
