    ...
    <Row :gutter="15" style="margin-bottom:20px;" type="flex">
      <Col style="width:100%">
        <top-card class="orginalmanage-topcard">
          ...
          <Row :gutter="15" style="margin-bottom:20px;" type="flex">
            <Col :span="4" class="ivu-form">
              <span class="ivu-form-item-label">资源清单</span>
            </Col>
            <Col :span="19">
              <Form
                ref="publicForm"
                ...
                :label-width="290"
              >
                <FormItem label="上传资源清单" prop="invertory">
                  <Table
...
import {
  PageOfficeDorganListAsync,
  CreateZhResourceInventoryAsync,
  UpdateZhResourceInventoryAsync,
  GetZhResourceInventoryDetails
...
export default {
  ...
  mounted() {
    ...
    console.log(id);
    if (id) {
      GetZhResourceInventoryDetails({ Id: id }).then(res => {
  ...
  methods: {
    loadData() {
      PageOfficeDorganListAsync().then(res => {
        this.current = res;
      });
    },
    edit(row) {
    ...
    submit() {
      this.$refs.publicForm.validate(valid => {
        if (valid) {
          ...
          let id = this.$route.query.obj;
          if (id) {
            UpdateZhResourceInventoryAsync(params).then(res => {
              this.$Message.success("资源清单更新成功");
              this.$router.push("/comprehensiveservice/resource/index");
            });
          } else {
            CreateZhResourceInventoryAsync(params).then(res => {
