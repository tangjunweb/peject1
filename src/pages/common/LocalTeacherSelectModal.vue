<template>
  <span @click="showModal" class="TeacherSelectModel">
    <slot>
      <Button size="large">选择讲师</Button>
    </slot>
    <Modal
      v-model="show"
      title="选择讲师"
        ...
        <Row>
          <Col :span="24" class="partymember-select">
            <Form :label-width="60" inline>
              <FormItem label="姓名">
                <Input v-model="searchParams.Name" placeholder="输入讲师姓名"></Input>
              </FormItem>
          ...
          <Col :span="7">
            <div class="party-member-right">
              <div class="party-member-head">
                <div class="party-member-hc">已选择讲师</div>
...
export default {
  ...
  methods: {
    ...
    //   }
      this.selectData =[];
      this.params.TeaSelectType = this.TeaSelectType;
      this.loadData();
      this.show = true;
    },
    del(i) {
    ...
    loadData() {
      ...
      PageSelectTeacherLibaryListAsync(this.ClearParams(params))
        .then(res => {
