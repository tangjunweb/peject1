...
import { TrainRecordPageList } from '@/api/education'
...
export default {
    ...
    data() {
        ...
        return {
            ...
            columns: [{
                ...
                tooltip: true,
                render (h,{row}) {
                    return h('span',that.GetCodeTableValue(row.classType,'ClassType'))
                }
            },{
                title: '开始时间',
                tooltip: true,
                render (h,{row}) {
                    return h('span',that.FormatDate(row.startTime,'yyyy-MM-dd hh:mm:ss'))
                }
            },{
                title: '结束时间',
                tooltip: true,
    ...
    computed: {
        title () {
            return `${this.partyMember.name}过往培训记录`;
        }
    },
    watch: {
        value (n) {
    ...
    methods: {
        loadData () {
            ...
            this.loading = true;
            TrainRecordPageList(this.ClearParams(params)).then(res=>{
