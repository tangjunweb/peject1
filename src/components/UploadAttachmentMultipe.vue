<template>
    ...
    >
        ...
        <span class="tips" style='font-size:16px;color:red;margin-left:20px' v-if='showTips && unExceed'>文件大小限制{{maxSize/1024}}M</span>
    ...
    methods: {
        ...
        download (el) {
            GetUpLoadAuth().then(res=>{
                window.open(`${UPLOAD_PATH}/file/download?project=${res.project}&token=${res.token}&id=${el.id}`)
            })
        }
    },
    mounted () {
